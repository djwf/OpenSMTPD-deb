...
gdir=~/usr/src/opensmtpd-portable
...
echo Building OpenSMTPD debian package based on git repository with latest commit:
echo
git log -n 1
echo
...
cp $odir/etc/default/opensmtpd $idir/etc/default/opensmtpd
mkdir -p $idir/etc/init.d
cp $odir/etc/init.d/opensmtpd $idir/etc/init.d/opensmtpd

# Place files in usr.
mkdir -p $idir/usr/share/doc/opensmtpd
cp $odir/usr/share/doc/opensmtpd/* $idir/usr/share/doc/opensmtpd/
mkdir -p $idir/usr/share/lintian/overrides
cp $odir/usr/share/lintian/overrides/opensmtpd $idir/usr/share/lintian/overrides

# Add README file from the git repository.
cp $gdir/README.md $idir/usr/share/doc/opensmtpd/
gzip -9 $idir/usr/share/doc/opensmtpd/README.md

# Add and rename LICENSE file from the git repository.
cp $gdir/LICENSE $idir/usr/share/doc/opensmtpd/copyright
...
fpm -ef -s dir -t deb -n opensmtpd -v $version -C $idir \
	-p $odir/opensmtpd-VERSION_ARCH.deb \
	-d "adduser" \
	-d "libc6 (>= 2.13-38)" \
	-d "libdb5.1" \
	...
	--after-install $odir/postinst \
	--before-install $odir/preinst \
	--after-remove $odir/postrm \
	--before-remove $odir/prerm \
